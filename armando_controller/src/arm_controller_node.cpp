...
class JointStateSub : public rclcpp::Node
{
public:
  JointStateSub()
  : Node("joint_state_subscriber")
  {
    subscription_ = this->create_subscription<sensor_msgs::msg::JointState>(
      "/joint_states", 10, std::bind(&JointStateSub::topic_callback, this, _1));
  }
...
public:
    ...
    step_(0)
  {
    ...
    timer_ = this->create_wall_timer(
      5000ms, std::bind(&PosControllerPublisher::timer_callback, this)
    );
  }
private:
...
int main(int argc, char * argv[])
{
  ...
  // Create nodes
  auto joint_sub = std::make_shared<JointStateSub>();
  auto controller_pub = std::make_shared<PosControllerPublisher>();
